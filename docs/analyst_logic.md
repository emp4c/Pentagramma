...
> The analyst emits the raw pivot-midpoint as `price`. At dispatch time the **coordinator** applies an entry stop floor: `effective_stop = min(price, fill_price × (1 − ENTRY_STOP_FLOOR_PCT))`. The analyst does not apply this floor — `status.active_stop_price` will be set to `effective_stop` by the coordinator, and the ratchet in subsequent LONG bars advances from there.
...
| `ENTRY_STOP_FLOOR_PCT` | 0.01 | Max 1% loss from fill price for the entry stop-loss; coordinator applies `min(pivot_midpoint, fill × (1 − pct))` at dispatch — analyst is unaffected |
...
Every `BUY_LIMIT` order carries a UUID generated by the analyst in its `condition` field. The paired `SELL_STOP` stop-loss references that UUID in its `condition` field as `on_fill:<uuid>`.
...
Analyst emits:
  ...
  SELL_STOP  → condition = "on_fill:<uuid>"  # references the BUY order by the same ID
```
...
- [ ] Handling of partial fills of SELL_STOP orders from the broker: keep the unfilled part as a SELL order and keep the machine status LONG until the entire position has been sold — in the meantime the same LONG logic is applied  
